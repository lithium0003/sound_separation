...
def make_randoms(length, amp, sample_rate):
    return np.random.rand(length) * 2 * amp - amp
    #return np.sin(np.linspace(0, length, length)*440/sample_rate*np.pi*2)*amp
...
class VoiceDataset():
    ...
    def get_next(self, dataset):
        ...
        offset_1 = np.squeeze(np.random.randint(0, len(mixture) - self.model.input_num_samples + 1, 1))

        mixture_fragment = mixture[offset_1:offset_1 + self.model.input_num_samples]
        batch_inputs = mixture_fragment

        batch_outputs = []
        for i in range(self.model.num_sources):
            vocal = self.sequences[dataset]['vocal%d'%(i+1)][sample_i]
            vocal_fragment = vocal[offset_1:offset_1 + self.model.input_num_samples]
